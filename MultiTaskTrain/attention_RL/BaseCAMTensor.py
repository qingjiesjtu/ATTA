# almost the same as in grad-cam package, only modify the output to tensor for backward on attention map
import numpy as np
import torch
import ttach as tta
from typing import Callable, List, Tuple
from .activations_and_gradients import ActivationsAndGradientsNotDetached
...
class BaseCAMTensor:
    ...
    def forward(self,
        ...
        target_categories = None # added by me if we set targets a priori
        if targets is None:
        ...
        if self.uses_gradients:
            self.model.zero_grad() #只是清零梯度，不会清空计算图！！
    ...
    def compute_cam_per_layer(
        ...
        for i in range(len(self.target_layers)):
            ...
            scaled = scale_cam_image(cam, target_size[::-1])
    ...
    def aggregate_multi_layers(
            self,
            cam_per_target_layer: np.ndarray) -> torch.Tensor:
